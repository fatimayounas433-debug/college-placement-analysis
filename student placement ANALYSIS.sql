...
------- NOW WE CAN IT WITH SECOND OPTION 
   SELECT 
   ...
   FROM COLLEGEPLACEMENT
    ...
    SELECT COLLEGE_ID, CGPA
FROM COLLEGEPLACEMENT
WHERE CGPA = (SELECT MIN(CGPA) FROM COLLEGEPLACEMENT)
   OR CGPA = (SELECT MAX(CGPA) FROM COLLEGEPLACEMENT);

  ------- NOW WE CAN USE JOIN 

SELECT C1.COLLEGE_ID AS COLLEGE_CGPA_LOWEST,
C1.CGPA AS LOWEST_CGPA,
C2.COLLEGE_ID AS HIGHEST_COLLEGE_CGPA, 
C2.CGPA AS HIGHEST_CGPA 
FROM COLLEGEPLACEMENT C1 
JOIN COLLEGEPLACEMENT C2 
WHERE C1.CGPA =(SELECT MIN(CGPA) FROM COLLEGEPLACEMENT)
AND C2.CGPA =(SELECT MAX(CGPA) FROM COLLEGEPLACEMENT);   

CALCULATE PLACMENT RATE PAR COLLEGE_ID 
SELECT 
    COLLEGE_ID,
    COUNT(*) AS total_students,
    SUM(CASE WHEN PLACEMENT = 'Yes' THEN 1 ELSE 0 END) AS placed_students,
      SUM(CASE WHEN PLACEMENT = 'NO' THEN 1 ELSE 0 END) AS NOT_placed_students,
    ROUND(100.0 * SUM(CASE WHEN PLACEMENT = 'Yes' THEN 1 ELSE 0 END) / COUNT(*), 2) AS placement_rate
FROM COLLEGEPLACEMENT
GROUP BY COLLEGE_ID;
